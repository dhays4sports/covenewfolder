(() => {
  ...
  function render() {
    ...
    if (progressRegion) {
      ...
      progressRegion.setAttribute('aria-valuetext', `Question ${current + 1} of ${questions.length}`);
    }
    if (questionLive && lastAnnouncedQuestion !== question.key) {
      questionLive.textContent = `Question ${current + 1} of ${questions.length}. ${visibleQuestion.title || question.title}`;
